...
{ //ω
	...
	if (nurpruefdb) {
		...
		if (!nurnachb && !nachbneu && !nurusmod && !pruefauft) {
			...
			systemrueck("Q="+ldatvz+"/;P="+fertigvz+"/;find /opt/turbomed/LaborStaber/DFUE/Update_Staber/LDTArchiv -iname '*.ldt' -newer \"$(printf $P;ls -t $P|head -n1)\" -exec sh -c { cp {} \"$Q\" \\; for a in $(find /DATA/Patientendokumente/ -mindepth 1 -maxdepth 1 -type d -regextype posix-extended -regex '.*/[^/]+Labor$'); do cp {} $a/; done; }",2);
			sleep(1);
